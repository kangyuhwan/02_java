 ...
 * 
 * 이미지 파일을 입력 받아서
 * 이미지 파일로 출력하는 (stream으로만 작업하는)
 * 
 * 이미지 복사 클래스
 * ----------------------------
 * -- 입력 --
 * 1. nodes stream(stream : FileInputStream)
...
public class ImageCopy {
	...
	public static void main(String[] args) throws IOException {
		...
		FileInputStream in = new FileInputStream("Penguins.jpg");
		
		// === 입력 node stream 선언, 초기화 완료
		
		// 3. 출력 node stream : FileOutputStream
		FileOutputStream out = new FileOutputStream("Penguins-copied.jpg");
		...
		// (2) read(byte[]) 메소드는 읽어들인 바이트 수 를 정수로 리턴
		//     읽은 데이터가 없으면 -1로 리턴
		int readSize = 0;
		int repeat = 0;
		...
		while ((readSize = in.read(buffer)) != -1) {
			//4. write(byte[]) 작업 : 
			// 출력 node stream 메소드로 작업
			out.write(buffer);
			// 화면에 작업 내용 출력
			System.out.printf("읽은 바이트 : %d\t, 반복 횟수 %d%n"
								, readSize,++repeat);
		}
