...
public class Account {
	...
	// 1. 멤버변수(실체변수) 선언부(메소드 외부인 클래스에서 선언)
	/** 계좌 번호 */
	int accNumber; 
	 ...
	 * 중복정의된 생성자
	 */
	Account(int newAccNumber, int newBalance)  { //yourAccount
	 ...
	 * 문자열로 된 계좌번호와
	 * 정수값의 잔액을 입력받아서
	 * 각각 계좌번호(accNumner)와 잔액(balance) 필드를 초기화
	 * 하는
	 * 중복정의된 생성자
	 * 
	 * 중복정의(Overloading)
	 * -----------------------------------------------
	 * 메소드의 이릉은 같고, 매개변수 목록이 다른 메소드를
	 * 정의하는 기법
	 ...
	 */
	/*Account(String newAccNumber, int newBalance)  { //yourAccount
		accNumber = Integer.valueOf(strAccNumber);
		balance = newBalance;
	}*/
	
	public void deposit(int amount) { //메소드 안에서 변수를 선언하면 지역변수
		balance += amount; //자동으로 0이 들어가 있다. 생성자가 생략됨
		// int value = 0;
		// value += amount; 
		// value 변수는 deposit메소드 안에서만 사용할 수 있다.
	}
	/**
	 * 이 계좌의 상태를 출력
	 */
	 ...
	 * 출금이 성공하면
	 * "얼마..가 출금되었습니다. 잔액은 얼마입니다."라는 문자열을 리턴한다.
	...
	public String withdraw(int amount) {
		...
		if (balance >= amount) {
			balance -= amount;
			result = amount + "가 출금되었습니다" + "잔액은" + "balacne" + "입니다.";
		} else {
			result = "잔액이 부족합니다.";
		}
		return result;	
